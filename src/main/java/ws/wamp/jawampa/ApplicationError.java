...
/**
 * Base class for all WAMP related exceptions that can/may be handled at the application
 * level.
 */
public class ApplicationError extends WampError {
	...
	public String toString() {
		...
		s.append("ApplicationError(").append(this.uri).append(", ");
	...
	/**
	 * Peer provided an incorrect URI for a URI-based attribute of a WAMP message such as
	 * a realm, topic or procedure.
	 */
	...
	/**
	 * A Dealer could not perform a unregister, since the given registration is not
	 * active.
	 */
	public final static String NO_SUCH_REGISTRATION = "wamp.error.no_such_registration";

	/**
	 * A Broker could not perform a unsubscribe, since the given subscription is not
	 * active.
	 */
	 ...
	 * performing payload validation checked the payload (args / kwargs) of a call, call
	 * result, call error or publish, and the payload did not conform.
	 */
	...
	/**
	 * A Dealer or Broker could not determine if the Peer is authorized to perform a join,
	 * call, register, publish or subscribe, since the authorization operation itself
	 * failed. E.g. a custom authorizer did run into an error.
	 */
	 ...
	 * the Router. For example, the Peer was successfully authenticated, but the Role
	 * configured does not exists - hence there is some misconfiguration in the Router.
	 */
	 ...
	 * registered, but Callee Black- and Whitelisting and/or Caller Exclusion lead to the
	 ...
	 * This might happen if the type of the connection configuration does not match to the
	 * type which the {@link IWampConnectorProvider} expects or if the value is null and
	 * the provider expects a non-null value.
	 */
	public final static String INVALID_CONNECTION_CONFIGURATION = "jawampa.error.invalid_connection_configuration";

	/**
	 * A scheduler that is not compatible to a particular connection provider was assigned
	 * to it
	 */
