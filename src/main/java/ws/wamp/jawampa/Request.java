...
/**
 * Holds the arguments for a WAMP remote procedure call and provides methods to send
 * responses to the caller.<br>
 * Either {@link #reply(ArrayNode, ObjectNode)} or
 * {@link #replyError(String, ArrayNode, ObjectNode)}} should be called in order to send a
 * positive or negative response back to the caller.
 */
public class Request {
	...
	static {
		replySentUpdater = AtomicIntegerFieldUpdater.newUpdater(Request.class,
				"replySent");
	}
	 ...
	 * If this is called more than once then the following invocations will have no
	 * effect. Respones will be only sent once.
	 * @param error The ApplicationError that shoul be serialized and sent as an
	 * exceptional response. Must not be null.
	 */
	...
	/**
	 * Send an error message in response to the request.<br>
	 * This version of the function will use Jacksons object mapping capabilities to
	 * transform the argument objects in a JSON argument array which will be sent as the
	 * positional arguments of the call. If keyword arguments are needed then this
	 * function can not be used.<br>
	 * If this is called more than once then the following invocations will have no
	 * effect. Respones will be only sent once.
	 * @param errorUri The error message that should be sent. This must be a valid WAMP
	 * Uri.
	...
	public void replyError(String errorUri, Object... args) throws ApplicationError {
		replyError(errorUri, ArgArrayBuilder.buildArgumentsArray(
				this.stateController.clientConfig().objectMapper(), args), null);
	}
	 ...
	 */
	public void replyError(String errorUri, ArrayNode arguments,
			ObjectNode keywordArguments) throws ApplicationError {
		...
		final ErrorMessage msg = new ErrorMessage(WampMessages.InvocationMessage.ID,
				this.requestId, null, errorUri, arguments, keywordArguments);

		this.stateController.tryScheduleAction(new Runnable() {
			@Override
			public void run() {
				if (Request.this.stateController.currentState() != Request.this.session)
					return;
				Request.this.session.connectionController().sendMessage(msg,
	 ...
	 * If this is called more than once then the following invocations will have no
	 * effect. Responses will be only sent once.
	 * @param arguments The positional arguments to sent in the response
	 * @param keywordArguments The keyword arguments to sent in the response
	 */
	public void reply(ArrayNode arguments, ObjectNode keywordArguments) {
		...
		final YieldMessage msg = new YieldMessage(this.requestId, null, arguments,
				keywordArguments);
