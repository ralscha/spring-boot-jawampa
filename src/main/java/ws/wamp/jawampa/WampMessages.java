...
public class WampMessages {
	...
	public static abstract class WampMessage {
		...
		/**
		 * A map which associates all message types which factories which can recreate
		 * them from received data.
		 */
	...
	/**
	 * Sent by a Client to initiate opening of a WAMP session to a Router attaching to a
	 * Realm. Format: [HELLO, Realm|uri, Details|dict]
	 */
	public static class HelloMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			...
			messageNode.add(ID);
			messageNode.add(this.realm);
			if (this.details != null)
				messageNode.add(this.details);
			else
	...
	/**
	 * Sent by a Router to accept a Client. The WAMP session is now open. Format:
	...
	public static class WelcomeMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			ArrayNode messageNode = mapper.createArrayNode();
			messageNode.add(ID);
			messageNode.add(this.sessionId);
			if (this.details != null)
				messageNode.add(this.details);
			else
				messageNode.add(mapper.createObjectNode());
	...
	public static class ChallengeMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			ArrayNode messageNode = mapper.createArrayNode();
			messageNode.add(ID);
			messageNode.add(this.authMethod);
			if (this.extra != null)
				messageNode.add(this.extra);
			else
				messageNode.add(mapper.createObjectNode());
			return messageNode;
		}
	...
	/**
	 * A Client having received a challenge is expected to respond by sending a signature
	 * or token. Format: [AUTHENTICATE, Signature|string, Extra|dict]
	 */
	public static class AuthenticateMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			ArrayNode messageNode = mapper.createArrayNode();
			messageNode.add(ID);
			messageNode.add(this.signature);
			if (this.extra != null)
				messageNode.add(this.extra);
			else
				messageNode.add(mapper.createObjectNode());
	...
	/**
	 * Sent by a Peer to close a previously opened WAMP session. Must be echo'ed by the
	 * receiving Peer. Format: [GOODBYE, Details|dict, Reason|uri]
	 */
	 ...
	 * Possible formats: [ERROR, REQUEST.Type|int, REQUEST.Request|id, Details|dict,
	 * Error|uri] [ERROR, REQUEST.Type|int, REQUEST.Request|id, Details|dict, Error|uri,
	 * Arguments|list] [ERROR, REQUEST.Type|int, REQUEST.Request|id, Details|dict,
	...
	public static class ErrorMessage extends WampMessage {
		...
		public ErrorMessage(int requestType, long requestId, ObjectNode details,
				String error, ArrayNode arguments, ObjectNode argumentsKw) {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			ArrayNode messageNode = mapper.createArrayNode();
			messageNode.add(ID);
			messageNode.add(this.requestType);
			messageNode.add(this.requestId);
			if (this.details != null)
				messageNode.add(this.details);
			else
				messageNode.add(mapper.createObjectNode());
			messageNode.add(this.error);
			if (this.arguments != null)
				messageNode.add(this.arguments);
			else if (this.argumentsKw != null)
				messageNode.add(mapper.createArrayNode());
			if (this.argumentsKw != null)
				messageNode.add(this.argumentsKw);
			return messageNode;
		}
		...
		static class Factory implements WampMessageFactory {
			...
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				...
				return new ErrorMessage(requestType, requestId, details, error, arguments,
						argumentsKw);
			}
		}
	}
	...
	/**
	 * Sent by a Publisher to a Broker to publish an event. Possible formats: [PUBLISH,
	 * Request|id, Options|dict, Topic|uri] [PUBLISH, Request|id, Options|dict, Topic|uri,
	 * Arguments|list] [PUBLISH, Request|id, Options|dict, Topic|uri, Arguments|list,
	 * ArgumentsKw|dict]
	 */
	public static class PublishMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			...
			if (this.options != null)
				messageNode.add(this.options);
			else
				messageNode.add(mapper.createObjectNode());
			messageNode.add(this.topic);
			if (this.arguments != null)
				messageNode.add(this.arguments);
			else if (this.argumentsKw != null)
				messageNode.add(mapper.createArrayNode());
	...
	public static class PublishedMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			...
			messageNode.add(ID);
			messageNode.add(this.requestId);
			messageNode.add(this.publicationId);
			return messageNode;
		}
	...
	public static class SubscribeMessage extends WampMessage {
		...
		static class Factory implements WampMessageFactory {
			@Override
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				if (messageNode.size() != 4 || !messageNode.get(1).canConvertToLong()
						|| !messageNode.get(2).isObject()
						|| !messageNode.get(3).isTextual())
					throw new WampError(ApplicationError.INVALID_MESSAGE);
	...
	public static class SubscribedMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			...
			messageNode.add(ID);
			messageNode.add(this.requestId);
			messageNode.add(this.subscriptionId);
			return messageNode;
		}
	 ...
	 * [EVENT, SUBSCRIBED.Subscription|id, PUBLISHED.Publication|id, Details|dict] [EVENT,
	 * SUBSCRIBED.Subscription|id, PUBLISHED.Publication|id, Details|dict,
	 * PUBLISH.Arguments|list] [EVENT, SUBSCRIBED.Subscription|id,
	 * PUBLISHED.Publication|id, Details|dict, PUBLISH.Arguments|list,
	...
	public static class EventMessage extends WampMessage {
		...
		static class Factory implements WampMessageFactory {
			...
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				...
				return new EventMessage(subscriptionId, publicationId, details, arguments,
						argumentsKw);
			}
		}
	}
	 ...
	 * Options|dict, Procedure|uri] [CALL, Request|id, Options|dict, Procedure|uri,
	 * Arguments|list] [CALL, Request|id, Options|dict, Procedure|uri, Arguments|list,
	 * ArgumentsKw|dict]
	 */
	public static class CallMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			...
			if (this.options != null)
				...
				messageNode.add(mapper.createObjectNode());
			messageNode.add(this.procedure);
			if (this.arguments != null)
				messageNode.add(this.arguments);
			else if (this.argumentsKw != null)
				messageNode.add(mapper.createArrayNode());
		...
		static class Factory implements WampMessageFactory {
			...
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				...
				return new CallMessage(requestId, options, procedure, arguments,
						argumentsKw);
			}
		}
	}
	...
	/**
	 * Result of a call as returned by Dealer to Caller. [RESULT, CALL.Request|id,
	 * Details|dict] [RESULT, CALL.Request|id, Details|dict, YIELD.Arguments|list]
	...
	/**
	 * A Callees request to register an endpoint at a Dealer. [REGISTER, Request|id,
	 * Options|dict, Procedure|uri]
	 */
	...
	/**
	 * Acknowledge sent by a Dealer to a Callee for successful registration. [REGISTERED,
	 * REGISTER.Request|id, Registration|id]
	 */
	public static class RegisteredMessage extends WampMessage {
		...
		public JsonNode toObjectArray(ObjectMapper mapper) throws WampError {
			...
			messageNode.add(ID);
			messageNode.add(this.requestId);
			messageNode.add(this.registrationId);
			return messageNode;
		}
	...
	/**
	 * A Callees request to unregister a previsouly established registration. [UNREGISTER,
	 * Request|id, REGISTERED.Registration|id]
	 * 
	 */
	 ...
	 * Request|id, REGISTERED.Registration|id, Details|dict] [INVOCATION, Request|id,
	 * REGISTERED.Registration|id, Details|dict, CALL.Arguments|list] [INVOCATION,
	 * Request|id, REGISTERED.Registration|id, Details|dict, CALL.Arguments|list,
	...
	public static class InvocationMessage extends WampMessage {
		...
		static class Factory implements WampMessageFactory {
			...
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				...
				return new InvocationMessage(requestId, registrationId, details,
	 ...
	 * INVOCATION.Request|id, Options|dict] [YIELD, INVOCATION.Request|id, Options|dict,
	 * Arguments|list] [YIELD, INVOCATION.Request|id, Options|dict, Arguments|list,
	 * ArgumentsKw|dict]
	 */
