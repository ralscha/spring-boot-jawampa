...
public class HandshakingState implements ClientState {
    ...
    void closeIncompleteSession(Throwable disconnectReason, String optAbortReason, boolean reconnectAllowed) {
        ...
        int nrReconnects = reconnectAllowed ? this.nrReconnectAttempts : 0;
        if (nrReconnects == 0) {
            this.stateController.setExternalState(new WampClient.DisconnectedState(disconnectReason));
        }
        WaitingForDisconnectState newState = new WaitingForDisconnectState(this.stateController, nrReconnects);
    ...
    void sendHelloMessage() {
        ...
        .sendMessage(new WampMessages.HelloMessage(this.stateController.clientConfig().realm(), this.stateController.clientConfig().helloDetails()), IWampConnectionPromise.Empty);
    }
    ...
    void onMessage(WampMessage msg) {
        ...
        if (msg instanceof WelcomeMessage) {
            ...
            SessionEstablishedState newState = new SessionEstablishedState(
                this.stateController, this.connectionController, sessionId, welcomeDetails, routerRoles);
            this.stateController.setState(newState);
        }
            ...
            List<ClientSideAuthentication> authMethods = this.stateController.clientConfig().authMethods();
            ...
            for (ClientSideAuthentication authMethod : authMethods) {
                if (authMethod.getAuthMethod().equals(authMethodString)) {
                    AuthenticateMessage reply =
                        authMethod.handleChallenge(challenge, this.stateController.clientConfig().objectMapper());
                    if (reply == null) {
                        handleProtocolError();
                    } else {
                        this.connectionController.sendMessage(reply, IWampConnectionPromise.Empty);
                    }
                    return;
                }
            }
