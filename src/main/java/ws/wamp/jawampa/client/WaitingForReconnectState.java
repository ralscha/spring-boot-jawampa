...
public class WaitingForReconnectState implements ClientState {
    ...
    public void onEnter(ClientState lastState) {
        this.reconnectSubscription =
            this.stateController.rxScheduler().createWorker().schedule(new Action0() {
                @Override
                public void call() {
                    if (WaitingForReconnectState.this.stateController.currentState() != WaitingForReconnectState.this) return;
                    // Reconnect now
                    ConnectingState newState = new ConnectingState(WaitingForReconnectState.this.stateController, WaitingForReconnectState.this.nrReconnectAttempts);
                    WaitingForReconnectState.this.stateController.setState(newState);
                }
    ...
    public void initClose() {
        ...
        DisconnectedState newState = new DisconnectedState(this.stateController, null);
