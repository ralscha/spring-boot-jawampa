...
/**
 * WampClientBuilder is the builder object which allows to create {@link WampClient}
 * objects.<br>
 * New clients have to be configured with the member methods before a client can be
 * created with the {@link #build build()} method.
 */
public class WampClientBuilder {
	...
	private List<WampSerialization> serializations = new ArrayList<>();
	private String authId = null;
	private List<ClientSideAuthentication> authMethods = new ArrayList<>();
	...
	/**
	 * The default reconnect interval in milliseconds.<br>
	 * This is set to 5s
	 */
	public static final int DEFAULT_RECONNECT_INTERVAL = 5000;
	/**
	 * The minimum reconnect interval in milliseconds.<br>
	...
	public WampClientBuilder() {
		...
		this.roles = EnumSet.of(WampRoles.Caller, WampRoles.Callee, WampRoles.Publisher,
	...
	/**
	 * Builds a new {@link WampClient WAMP client} from the information given in this
	 * builder.<br>
	 * At least the uri of the router and the name of the realm have to be set for a
	 * proper operation.
	...
	public WampClient build() throws Exception {
		...
		try {
			UriValidator.validate(this.realm, this.useStrictUriValidation);
		}
		catch (Exception e) {
			throw new ApplicationError(ApplicationError.INVALID_REALM);
		}
		...
		WampRoles[] rolesArray = new WampRoles[this.roles.size()];
		int i = 0;
		for (WampRoles r : this.roles) {
			rolesArray[i] = r;
			i++;
		}
		...
		// This can throw!
		IWampConnector connector = this.connectorProvider.createConnector(routerUri,
				this.connectionConfiguration, this.serializations);

		// Use default object mapper
		if (this.objectMapper == null)
			this.objectMapper = new ObjectMapper();

		ClientConfiguration clientConfig = new ClientConfiguration(this.closeOnErrors, this.authId,
				this.authMethods, routerUri, this.realm, this.useStrictUriValidation, rolesArray,
				this.nrReconnects, this.reconnectInterval, this.connectorProvider, connector,
				this.objectMapper);
	 ...
	 * @param roles The set of roles that the client should fulfill in the session. At
	 * least one role is required, otherwise the session can not be established.
	 * @return The {@link WampClientBuilder} object
	 */
	 ...
	 * The type of this configuration data depends on the used
	 * {@link IWampConnectorProvider}.<br>
	 ...
	 * @return The {@link WampClientBuilder} object
	 */
	public WampClientBuilder withConnectionConfiguration(
	 ...
	 * negotiation.<br>
	 * Use this function to adjust the serializations if not all are needed or a different
	 * order is desired.
	 ...
	 * strict URI validation rules which are described in the WAMP specification. By
	 * default the loose Uri validation rules will be used.
	...
	/**
	 * Sets whether the client should be closed when an error besides a connection loss
	 * happens.<br>
	 * Other reasons are the reception of invalid messages from the remote or the abortion
	 * of the session by the remote.<br>
	 * When this flag is set to true no reconnect attempts will be performed. <br>
	 * The default is true. This is to avoid endless reconnects in case of a
	 * malfunctioning remote.
	 * @param closeOnErrors True if the client should be closed on such errors, false if
	 * not.
	 * @return The {@link WampClientBuilder} object
	 */
	...
	/**
	 * Sets the amount of time that should be waited until a reconnect attempt is
	 * performed.<br>
	 ...
	 */
	public WampClientBuilder withReconnectInterval(int interval, TimeUnit unit)
			throws ApplicationError {
	...
	/**
	 * Set custom, pre-configured {@link ObjectMapper}. This instance will be used instead
	 * of default for serialization and deserialization.
